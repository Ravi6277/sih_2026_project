  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-gray-500 mt-1">Track patient follow-up appointments.</p>
      </motion.div>

      {/* Summary cards */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Upcoming', count: followups.filter(f => f.status === 'upcoming').length, color: 'text-amber-600 bg-amber-50' },
          { label: 'Missed', count: followups.filter(f => f.status === 'missed').length, color: 'text-rose-600 bg-rose-50' },
          { label: 'Completed', count: followups.filter(f => f.status === 'completed').length, color: 'text-sahaay-deep bg-sahaay-surface' },
        ].map((s, i) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  {fu.status === 'upcoming' && (
                    <Button size="sm" onClick={() => showToast('Reminder sent to patient')}>Remind</Button>
                  )}
                  {fu.status === 'missed' && (
